...
def _preprocess_data(data):
    ...
    feature_vector_df['Valencia_pressure'] = feature_vector_df['Valencia_pressure'].astype(np.float64)
    ...
    feature_vector_df = feature_vector_df.drop(['Unnamed:_0'], axis=1)

    pressures = []
    for T in feature_vector_df['Seville_temp']:
        p_h = 101325 * np.exp(
            (-5.6e-26 * 9.8 * 680) / (1.380649e-23 * float(T))) * 0.01  # where T is a temperature of a particular day
        pressures.append(p_h + 100)
    feature_vector_df['Seville_pressure'] = pressures
    ...
    feature_vector_df['weekday'] = feature_vector_df['time'].dt.weekday
    # week of the year
    feature_vector_df['week_of_year'] = feature_vector_df['time'].dt.week
